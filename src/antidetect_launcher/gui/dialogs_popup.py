...
def show_notes_edit_popup(
    parent, notes: str, note_templates: list[tuple[str, str]] | None = None
) -> str | None:
...
def show_proxy_pool_popup(parent, proxies: list) -> list | None:
    ...
    proxy_input.setPlaceholderText("host:port\nhost:port:user:pass\nsocks5://host:port")
    ...
    def add_proxies():
        ...
        if errors:
            alert.show_error("Parse Errors", f"{len(errors)} line(s) failed to parse.")
...
def show_profile_popup(
    parent, profile: BrowserProfile | None = None, storage=None
) -> tuple[BrowserProfile, bool] | None:
    ...
    if storage:
        ...
        def select_from_pool():
            ...
            item, ok = get_item_dialog(
                parent, "Select Proxy from Pool", "Choose proxy:", items, 0, False
            )
...
def show_settings_popup(parent, settings) -> bool:
    ...
    def browse_browser():
        file_path, _ = QFileDialog.getOpenFileName(
            parent, "Select Browser Executable", "", "Executables (*)"
        )
        if file_path:
    ...
    def add_addon():
        file_path, _ = QFileDialog.getOpenFileName(
            parent, "Select Extension File", "", "Firefox Extensions (*.xpi)"
        )
        if file_path:
    ...
    def on_save():
        ...
        set_autostart(autostart_checkbox.isChecked())

        settings.custom_addons = [
            custom_addons_list.item(i).text() for i in range(custom_addons_list.count())
        ]
